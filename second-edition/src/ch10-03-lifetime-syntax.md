...
Lifetimes являются уникальной парадигмой языка Rust. Эта тема настолько обширна, что
...
находиться в зоне своего объявления. Как же в таком случае компилятор Rust понимает,
что содержание переменной уже недействительно?

#### Проверка заимствования

В состав компилятора Rust входит функциональность называющаяся *проверка заимствования*.
...
В данном примере переменная `x` имеет время жизни `'b`, которое больше, чем время жизни
`'a`. Это означает, что переменная `r` может ссылаться на переменную `x` и её значение
будет действительно при выведении значения на консоль и до конца блока, в котором
...
для того, чтобы хорошо представлять особенности и тонкости рассматриваемого вопроса
в данном примере.
...
Описание ошибки давольно информативно, но всё-таки не очень понятно.
...
устранить неопределенность и дать возможность компилятору провести все необходимые
проверки.

### Синтаксис описания времени жизни переменных

Описания времени жизни не может изменить время жизни ссылок в программе.
Так же как функции могут принимать любые входные данные удовлетворяющие условиям,
...
как обобщенные параметры времени жизни изменяемых переменных связаны между собой.
...
внимание, что аннотации в функциях располагаются в треугольных скобках, так же,
как и обобщенные типы данных. Ограничение которое мы хотим внести в данную
...
поредения времени жизни*. Эти правила существуют для компилятора, а не для пишущего
...
Второе правило применяется т.к. здесь только один входной параметр времени
...
Второе правило нельзя применить, поскольку входных параметров несколько. Смотрим
...
типов данных. Типажи и связывании с типажами помогают соблюсти конвенции и контракты
